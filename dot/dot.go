...
// Package dot provides convenience wrappers for fluent's element constructors,
// for use with Go's dot import. Each wrapper forwards to an element package's New
// constructor: Div(children...) calls div.New(children...) and Svg(children...)
// calls svg.New(children...), so markup composes without package prefixes.
//
// Elements with specialised constructors (such as meta.UTF8) are reached by
// importing their package directly.
package dot
...
// Svg creates the root <svg> element. It renders the SVG namespace
// (xmlns="http://www.w3.org/2000/svg") by default and accepts SVG shapes as
// children.
func Svg(children ...svg.Shape) *svg.Element {
	return svg.New(children...)
}
