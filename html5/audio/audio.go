...
//
// The <audio> element embeds an audio player. Use src for one resource or
// source children for alternative encodings. Controls requests the browser's
// playback interface. Autoplay and preload remain subject to browser policy.
package audio
...
// PreloadAuto creates a audio element that allows the browser to preload the
...
// PreloadMetadata creates a audio element that requests metadata, such as
...
// PreloadNone creates a audio element that requests no advance fetching.
...
//
// Requests playback as soon as enough audio is available. Browser autoplay
// policy may require user interaction before audible playback is allowed.
//
...
//
// When present, the browser displays built-in audio controls allowing users to control playback.
//
// Calling without an argument sets the attribute. Pass a bool to control presence conditionally.
...
//
// A Boolean attribute that causes the audio to automatically restart from the beginning when it reaches the end.
//
// Calling without an argument sets the attribute. Pass a bool to control presence conditionally.
...
//
// A Boolean attribute that indicates whether the audio is initially silenced.
//
// Calling without an argument sets the attribute. Pass a bool to control presence conditionally.
...
//
// Controls Cross-Origin Resource Sharing (CORS) behaviour when fetching the audio file.
...
//
// Disables remote playback capabilities on connected devices (Miracast, Chromecast, DLNA, AirPlay).
//
// Calling without an argument sets the attribute. Pass a bool to control presence conditionally.
...
//
// Controls how the browser loads the audio element. Use lazy loading to defer loading until the element
...
//
// Hints how much audio to fetch before playback: none, metadata or auto.
