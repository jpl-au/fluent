...
// Controls When present, the browser displays built-in audio controls allowing users to control playback.
//
// Calling without an argument sets the attribute. Pass a bool to control presence conditionally.
...
// Loop A Boolean attribute that causes the audio to automatically restart from the beginning when it reaches the end.
//
// Calling without an argument sets the attribute. Pass a bool to control presence conditionally.
...
// ControlsList Customizes which controls are displayed when the browser shows its built-in audio controls.
func (e *element) ControlsList(list string) *element {
	e.controlsList = list
	return e
}

// CrossOrigin Controls Cross-Origin Resource Sharing (CORS) behavior when fetching the audio file.
func (e *element) CrossOrigin(value crossorigin.CrossOrigin) *element {
	e.crossorigin = value
	return e
}

// DisableRemotePlayback Disables remote playback capabilities on connected devices (Miracast, Chromecast, DLNA, AirPlay).
//
// Calling without an argument sets the attribute. Pass a bool to control presence conditionally.
...
// Loading Controls how the browser loads the audio element. Use lazy loading to defer loading until the element
...
// Preload Provides a hint to the browser about optimal preloading behaviour for the best user experience.
