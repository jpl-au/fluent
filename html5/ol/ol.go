...
//
// The <ol> element contains a list whose order is significant, with li children
// for its entries. Start, reversed and type control numbering. Use ul when
// reordering the entries would not change their meaning.
package ol
...
// Element represents the <ol> HTML element.
type Element struct {
...
// ItemsOf creates a new <ol> element from a slice of data and a mapper that returns one li element per item, enforcing correct nesting at compile time.
...
// Example: ol.ItemsOf([]int{1, 2}, func(int) *li.Element { return li.New() })
// Renders: <ol><li></li><li></li></ol>
...
// ItemsFunc creates a new <ol> element from a function that returns li elements, enforcing correct nesting at compile time.
...
// Example: ol.ItemsFunc(func() []*li.Element { return []*li.Element{li.New(), li.New()} })
// Renders: <ol><li></li><li></li></ol>
...
//
// Numbers the list items in reverse order (counting down instead of up).
//
// Calling without an argument sets the attribute. Pass a bool to control presence conditionally.
func (e *Element) Reversed(conds ...bool) *Element {
	if len(conds) > 0 {
		e.reversed = conds[0]
	} else {
		e.reversed = true
...
//
// Defines the numbering style used for list item markers
func (e *Element) Type(value listtype.ListType) *Element {
	e.listtype = value
	return e
}
