...
//
// The <ins> HTML element represents a range of text that has been added to a document. You can use the <del>
...
// Text creates a new ins element with text content. Uses text.Text which HTML-escapes the output.
...
// when the insertion occurred. Uses text.Text which HTML-escapes the output.
...
// reason for the insertion. Uses text.Text which HTML-escapes the output.
...
// timestamp. Use when recording both why and when content was added.
...
//
// A URI for a resource that explains or documents the reason for the insertion. This could link to meeting
// minutes, change logs, bug reports, or any documentation that provides context for why the content was
// added. Useful for maintaining an audit trail of content changes and providing transparency in
// collaborative editing scenarios.
func (e *Element) Cite(url string) *Element {
	e.cite = node.EscapeAttribute(url)
	return e
}

// DateTime sets the datetime attribute.
//
// This attribute indicates the time and date when the insertion occurred. The value must be a valid date
// string with an optional time component in ISO 8601 format (e.g., '2023-12-25', '2023-12-25T10:30:00',
// or '2023-12-25T10:30:00Z'). If the value cannot be parsed as a valid date-time string, the element does
// not have an associated timestamp. This is valuable for version control, auditing, and displaying change
// history to users.
func (e *Element) DateTime(datetime string) *Element {
	e.dateTime = node.EscapeAttribute(datetime)
	return e
}
...
func (e *Element) AttributeBuilder(buf *bytes.Buffer) {
	if e.cite != "" {
		buf.Write(html5.AttrCite)
		buf.WriteString(e.cite)
		buf.Write(html5.MarkupQuote)
	}
	if e.dateTime != "" {
		buf.Write(html5.AttrDateTime)
		buf.WriteString(e.dateTime)
		buf.Write(html5.MarkupQuote)
	}
	if e.class != "" {
