...
//
// The <canvas> element provides a drawing surface for APIs such as Canvas 2D
// and WebGL. Width and height define the drawing dimensions. CSS controls its
// displayed size. Child content supplies a fallback representation of the drawing.
package canvas
...
// Element represents the <canvas> HTML element.
type Element struct {
...
//
// The height of the canvas drawing space in CSS pixels, defaulting to 150.
// This is separate from CSS height, which controls display size. Changing the
// drawing dimensions resets the bitmap and rendering context state.
...
//
// The width of the canvas drawing space in CSS pixels, defaulting to 300.
// This is separate from CSS width, which controls display size. Changing the
// drawing dimensions resets the bitmap and rendering context state.
...
func (e *Element) RenderBuilder(buf *bytes.Buffer) {
	...
	buf.Write(html5.TagCanvasClose)
}
