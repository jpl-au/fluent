...
//
// The <object> HTML element represents an external resource, which can be treated as an image, a nested
...
func (e *Element) Type(mime string) *Element {
	e.objectType = node.EscapeAttribute(mime)
	return e
}

// Form sets the form attribute.
//
// Associates the object element with a form element in the document
func (e *Element) Form(formID string) *Element {
	e.form = node.EscapeAttribute(formID)
	return e
}
...
//
// Assigns a name to the embedded content for targeting and reference purposes
...
//
// Associates the object with an image map defined by a <map> element
//
// Deprecated: Image maps with object are no longer supported.
...
func (e *Element) RenderBuilder(buf *bytes.Buffer) {
	...
	buf.Write(html5.TagObjectClose)
}
