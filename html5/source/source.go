...
//
// The <source> element offers an alternative resource to picture, audio or
// video. Use src for audio and video resources, or srcset for images inside
// picture. Type lets the browser skip formats it cannot use.
package source
...
func ImageWebP(srcset string) *Element {
	return &Element{
		mime:   "image/webp",
		srcset: node.EscapeAttribute(srcset),
	}
}
...
//
// Specifies the URL of the media resource for <audio> and <video> elements. This can be an absolute URL or a
// relative path to the media file. When the <source> element is used inside a <picture> element, this
// attribute is ignored. Use srcset instead. Multiple source elements with different src values
// enable fallback options for different browser capabilities.
...
//
// Specifies the MIME media type of the resource, helping browsers determine if they can play or display the
// content before downloading it. Can include codec parameters for precise format specification. Examples:
// 'video/mp4; codecs="avc1.42E01E"', 'image/webp', 'audio/ogg; codecs=vorbis'. This enables browsers to
// skip unsupported formats and choose the best available option efficiently.
...
//
// Describes the image's expected rendered width when this source is used in a
// picture with srcset width descriptors. For example, '(max-width: 600px) 480px,
// 800px' declares 480px up to that viewport breakpoint and 800px otherwise.
// This helps select an image resource. CSS still determines its layout width.
...
//
// Defines the media query conditions under which this source is eligible for selection, using CSS media query syntax.
// Only used when the <source> element is a direct child of a <picture> element. Enables art direction by
// serving different images based on viewport size, device orientation, or display characteristics. Example:
// '(max-width: 600px)' or '(orientation: portrait)'.
func (e *Element) Media(mediaQuery string) *Element {
	e.media = node.EscapeAttribute(mediaQuery)
	return e
}

// Width sets the width attribute.
//
// Specifies the intrinsic width of the image resource in pixels as an integer without units. This helps
// browsers calculate aspect ratios and prevent layout shifts during image loading. When used with height,
...
//
// Specifies the intrinsic height of the image resource in pixels as an integer without units. Combined with
// the width attribute, this enables browsers to calculate the correct aspect ratio and allocate appropriate
