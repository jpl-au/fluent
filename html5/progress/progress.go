...
//
// The <progress> HTML element displays a visual indicator showing task completion progress, typically
// rendered as a progress bar. The value attribute indicates current progress, while max defines the target.
// Used for file uploads, form completion, loading states, and any measurable task progress. Content within
// the element serves as fallback text for browsers that don't support the element.
package progress
...
//
// Specifies the current completion amount of the task. Must be a valid floating-point number between 0 and
...
//
// Defines the maximum value that represents the complete state of the task. Must be a positive floating-point
...
func (e *Element) RenderBuilder(buf *bytes.Buffer) {
	...
	buf.Write(html5.TagProgressClose)
}
