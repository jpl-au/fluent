...
//
// The <form> element groups controls for submission. Action, method and enctype
// determine the destination and encoding of submitted data. Controls can belong
// to the form through nesting or an explicit form attribute.
package form
...
// Renders: <form method="dialog"><button type="submit">Confirm</button><button type="button">Cancel</button></form>
...
//
// The destination URL for form submission.
//
// Fluent filters this URL at render time: http, https, mailto, tel, sms and relative URLs
// render as given; any other scheme (javascript:, data:, file:, a custom protocol) renders as the
// inert node.UnsafeURL sentinel instead. For a trusted off-allowlist value use SetAttribute
...
//
// The submission method. The get method adds form entries to the destination URL.
// The post method sends them in the request body. The dialog method closes the containing dialog
// without sending a request.
func (e *Element) Method(value method.Method) *Element {
	e.method = value
	return e
}
...
//
// The character encoding used for form submission. UTF-8 is the conforming
// value for this attribute.
func (e *Element) AcceptCharset(value charset.Charset) *Element {
	e.acceptcharset = value
	return e
}

// AutoCapitalize sets the autocapitalize attribute.
//
// Controls whether and how text input is automatically capitalised as the user enters or edits it.
...
//
// The encoding for a post submission. Use multipart/form-data when uploading
// files. The default is application/x-www-form-urlencoded. A submit control's
// formenctype can override this value.
func (e *Element) EncType(value enctype.EncType) *Element {
	e.enctype = value
	return e
}
...
//
// A name for the form. This name must be unique among forms in the same document.
func (e *Element) Name(value string) *Element {
	e.name = node.EscapeAttribute(value)
	return e
}

// NoValidate sets the novalidate attribute.
//
// A boolean attribute that bypasses interactive constraint validation on submission.
//
// Calling without an argument sets the attribute. Pass a bool to control presence conditionally.
func (e *Element) NoValidate(conds ...bool) *Element {
	if len(conds) > 0 {
		e.novalidate = conds[0]
	} else {
		e.novalidate = true
	}
	return e
...
//
// A space-separated list describing the relationship between the document and
// the form's destination, such as search or help. Noopener and noreferrer also
// control aspects of navigation when the form opens another browsing context.
func (e *Element) Rel(value ...rel.Rel) *Element {
	if len(value) == 0 {
		return e
	}
	if len(e.rel) == 0 {
...
func (e *Element) Target(value target.Target) *Element {
	e.target = value
	return e
}
...
func (e *Element) AttributeBuilder(buf *bytes.Buffer) {
	if e.action != "" {
		buf.Write(html5.AttrAction)
		buf.WriteString(e.action)
		buf.Write(html5.MarkupQuote)
	}
	if len(e.method) > 0 {
		buf.Write(html5.AttrMethod)
		buf.Write(e.method)
		buf.Write(html5.MarkupQuote)
	}
	if e.name != "" {
	...
	if len(e.acceptcharset) > 0 {
		buf.Write(html5.AttrAcceptCharset)
		buf.Write(e.acceptcharset)
		buf.Write(html5.MarkupQuote)
	}
	if len(e.autocapitalize) > 0 {
		buf.Write(html5.AttrAutoCapitalize)
		buf.Write(e.autocapitalize)
		buf.Write(html5.MarkupQuote)
	}
	if len(e.enctype) > 0 {
		buf.Write(html5.AttrEncType)
		buf.Write(e.enctype)
		buf.Write(html5.MarkupQuote)
	}
	...
	if e.novalidate {
		buf.Write(html5.AttrNoValidate)
	}
	if len(e.rel) > 0 {
