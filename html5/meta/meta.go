...
// Content Contains the actual value of the metadata property specified by the name attribute. This is the substantive information being provided - the page description, author name, keyword list, or other metadata value. The format and expected content varies depending on the name attribute, from simple text strings to complex structured values like viewport configurations.
...
// HttpEquiv Defines HTTP header-equivalent directives that affect how the document is processed or displayed. This attribute allows HTML documents to specify certain HTTP header behaviors directly in the markup. Common values include 'refresh' for automatic page refreshing, 'content-type' for MIME type specification, 'X-UA-Compatible' for Internet Explorer compatibility modes, and 'Content-Security-Policy' for security policies.
func (e *element) HttpEquiv(directive string) *element {
	e.httpEquiv = directive
	return e
}

// Scheme Defines a scheme or format for interpreting the metadata content value. This attribute has been deprecated in HTML5 and should not be used in modern web development. It was originally intended to provide context for how to interpret metadata values, but modern approaches use standardized name values and structured content formats instead.
//
