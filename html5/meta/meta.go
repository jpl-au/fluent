...
//
// The <meta> HTML element provides metadata about the document that cannot be represented by other elements
// like base, link, script, style, or title. Used for character encoding, viewport configuration, page
// descriptions, author information, keywords, Open Graph data, and HTTP-equivalent headers. Essential for
// SEO, social media sharing, mobile responsiveness, and document properties.
package meta
...
func ThemeColor(color string) *Element {
	return &Element{
		name:    "theme-color",
		content: node.EscapeAttribute(color),
	}
}
...
func CSP(policy string) *Element {
	return &Element{
		...
		content:   node.EscapeAttribute(policy),
	}
}
...
//
// Contains the actual value of the metadata property specified by the name attribute. This is the substantive information being provided - the page description, author name, keyword list, or other metadata value. The format and expected content varies depending on the name attribute, from simple text strings to complex structured values like viewport configurations.
func (e *Element) Content(content string) *Element {
	e.content = node.EscapeAttribute(content)
	return e
}

// Charset sets the charset attribute.
//
// Declares the character encoding used for the HTML document, which must match the actual encoding of the file. This critical declaration should appear early in the document (within the first 1024 bytes) to ensure proper text interpretation. UTF-8 is the recommended encoding for modern web documents as it supports all Unicode characters while maintaining compatibility with ASCII.
func (e *Element) Charset(charset charset.Charset) *Element {
	e.charset = charset
	return e
}
...
//
// Defines a scheme or format for interpreting the metadata content value. This attribute has been deprecated in HTML5 and should not be used in modern web development. It was originally intended to provide context for how to interpret metadata values, but modern approaches use standardized name values and structured content formats instead.
//
// Deprecated: This attribute has been removed from the HTML5 specification.
func (e *Element) Scheme(scheme string) *Element {
	e.SetAttribute("scheme", scheme)
	return e
}

// Property sets the property attribute.
//
// Specifies the property name for RDFa (Resource Description Framework in attributes) metadata, most commonly used for Open Graph protocol data that enables rich social media sharing. Open Graph properties like 'og:title', 'og:description', 'og:image', and 'og:url' control how content appears when shared on social platforms like Facebook, Twitter, and LinkedIn. Also used for other structured metadata vocabularies like Schema.org and Dublin Core.
func (e *Element) Property(property string) *Element {
	e.property = node.EscapeAttribute(property)
	return e
}

// Media sets the media attribute.
//
// Specifies a media query indicating which media the metadata applies to. Commonly used with name="theme-color"
...
func (e *Element) Media(query string) *Element {
	e.media = node.EscapeAttribute(query)
	return e
}
...
func (e *Element) AttributeBuilder(buf *bytes.Buffer) {
	...
	if e.content != "" {
		buf.Write(html5.AttrContent)
		buf.WriteString(e.content)
		buf.Write(html5.MarkupQuote)
	}
	if len(e.charset) > 0 {
		buf.Write(html5.AttrCharset)
		buf.Write(e.charset)
		buf.Write(html5.MarkupQuote)
	}
	if e.httpEquiv != "" {
