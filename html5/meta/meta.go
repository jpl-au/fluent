...
//
// The <meta> element supplies document metadata not represented by elements
// such as title or link. Use charset for encoding, name and content for named
// metadata, or http-equiv and content for supported document directives.
package meta
...
//
// The metadata name whose value is provided by content, such as description,
// author, viewport or theme-color. Each name defines its own value format and
// meaning.
...
//
// The value for the metadata name or http-equiv directive on this element.
// Its syntax depends on that name or directive, such as plain text for a
// description or a list of settings for viewport.
...
//
// Declares the document's character encoding. The declaration must match the
// file's encoding and fit entirely within the first 1024 bytes of the document.
// Use UTF-8 for HTML documents.
...
//
// Obsolete attribute formerly used to describe the format of a metadata value.
// Use the value syntax defined by the selected metadata name instead.
//
...
// to provide different theme colours for light and dark mode, allowing responsive metadata based on user
