...
// Warning: HTML escaping breaks CSS syntax (e.g., > in selectors). Use RawText, Static, or CSS instead.
...
// Warning: HTML escaping breaks CSS syntax. Use RawTextf instead.
...
//
// Identifies the stylesheet language. Omit this attribute for CSS. The legacy
// value is text/css. Other values do not make browsers support another
// stylesheet language.
...
//
// Specifies the media types and conditions in which the enclosed CSS applies, using CSS media
// query syntax. This enables responsive design and device-specific styling directly within the style element.
// Examples: 'screen', 'print', 'screen and (max-width: 600px)', '(prefers-color-scheme: dark)'. When
// omitted, styles apply to all media types.
...
//
// Supplies the nonce used by the document's Content Security Policy to authorise
// this inline stylesheet. It must match the nonce in the applicable style-src
// or style-src-elem directive. Generate a fresh, unpredictable nonce for each
// response and use the same value in the policy and authorised elements.
...
//
// Names a stylesheet set. Sheets with the same title belong to the same set.
// The document's preferred or selected set determines which titled sheets apply.
// An untitled sheet is persistent rather than part of a named set. A title alone
// does not create an alternate stylesheet or guarantee a browser selection UI.
// Example: style.RawText("body { background: black; color: white; }").Title("Dark Theme")
func (e *Element) Title(title string) *Element {
	e.title = node.EscapeAttribute(title)
	return e
}

// Blocking sets the blocking attribute.
//
// Requests that the browser block the specified operations until the stylesheet is loaded and applied.
// Currently only supports the "render" value which blocks rendering until styles are applied,
