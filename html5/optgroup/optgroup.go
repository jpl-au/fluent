...
//
// The <optgroup> element groups related options within a select. Label names
// the group, and disabled makes all options in that group unavailable.
package optgroup
...
// Element represents the <optgroup> HTML element.
type Element struct {
...
// OptionsOf creates a new <optgroup> element from a slice of data and a mapper that returns one option element per item, enforcing correct nesting at compile time.
...
// Example: optgroup.OptionsOf([]int{1, 2}, func(int) *option.Element { return option.New() })
// Renders: <optgroup><option></option><option></option></optgroup>
...
// OptionsFunc creates a new <optgroup> element from a function that returns option elements, enforcing correct nesting at compile time.
...
// Example: optgroup.OptionsFunc(func() []*option.Element { return []*option.Element{option.New(), option.New()} })
// Renders: <optgroup><option></option><option></option></optgroup>
...
func Labelled(label string, options ...*option.Element) *Element {
	...
	return &Element{
		nodes: nodes,
		label: node.EscapeAttribute(label),
	}
}

// Label sets the label attribute.
//
// Specifies the name or title of the option group that browsers display as a heading above the grouped options.
...
//
// Disables the entire option group, making all options within it unselectable.
//
...
func (e *Element) RenderBuilder(buf *bytes.Buffer) {
	...
	buf.Write(html5.TagOptgroupClose)
}
