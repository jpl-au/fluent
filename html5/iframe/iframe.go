...
//
// The <iframe> element embeds another document in its own browsing context.
// Src loads a URL and srcdoc supplies inline HTML. Sandbox and allow control
// restrictions and permitted features. An iframe is not sandboxed merely because
// its content is embedded.
package iframe
...
// The browser can defer loading until the iframe is near the viewport.
//
...
//
// Sets the embedded viewport's height in CSS pixels. The embedded document
// controls how overflowing content is presented, including whether it scrolls.
...
//
// Hints when to load the iframe. Eager requests normal loading.
// Lazy permits the browser to defer it until near the viewport. The browser
// chooses the distance and timing.
...
//
// Sets a Permissions Policy for the embedded document. Separate feature
// directives with semicolons, for example "camera 'none'; microphone 'self';
// geolocation *". This can further restrict the parent's policy. It cannot
// override that policy to grant a denied feature. User permission may still be required.
...
//
// Allows the embedded document to request fullscreen through the Fullscreen
// API, subject to the applicable Permissions Policy and browser requirements.
// The allow attribute can also configure fullscreen permission.
//
...
//
// The name of this browsing context. Links and forms can use the same value
// in target to navigate the iframe instead of the current page.
...
//
// An HTML document to display inside the iframe. It takes precedence over src.
// Sandbox restrictions apply only when configured. The srcdoc attribute alone does not
// isolate the embedded document from the parent.
...
//
// Loads the iframe using an ephemeral context that does not use its origin's
// normal cookies or storage. Network requests are still possible. Support and
// interaction with embedding policies depend on the browser.
//
// Calling without an argument sets the attribute. Pass a bool to control presence conditionally.
