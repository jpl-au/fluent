...
//
// The <iframe> HTML element represents a nested browsing context, embedding another HTML page within the
// current document. Creates an isolated environment for displaying external content, third-party widgets,
// embedded applications, or sandboxed content. Supports various security controls via sandbox and permissions
// attributes. Common uses include embedded videos, maps, payment forms, and external content integration.
package iframe
...
//
// The width of the iframe in CSS pixels. This sets the horizontal display size of the embedded content area. While this can be specified as a number (interpreted as pixels), modern web development typically uses CSS for responsive sizing. The actual content may be scaled or scrolled if it doesn't fit the specified dimensions.
...
//
// The height of the iframe in CSS pixels. This sets the vertical display size of the embedded content area. Similar to width, this creates a fixed-size viewport for the embedded content. Content that exceeds these dimensions will be scrollable within the iframe, unless scrolling is disabled via CSS or other attributes.
...
//
// Controls the loading behavior of the iframe to optimize page performance. This attribute helps with page load optimization and user experience.
// Possible values: eager (load immediately), lazy (defer loading until needed, typically when scrolled into view)
func (e *Element) Loading(value loading.Loading) *Element {
	e.loading = value
	return e
}
...
//
// Specifies the permissions policy (Feature Policy) for the iframe, controlling which browser features the
// embedded content can access. This is a security mechanism that can restrict access to APIs like camera,
// microphone, geolocation, payment, etc. Values are semicolon-separated feature directives like
// 'camera none; microphone self; geolocation *'. Essential for maintaining security boundaries between
// parent and embedded content.
func (e *Element) Allow(policy string) *Element {
	e.SetAttribute("allow", policy)
	return e
}

// AllowFullscreen sets the allowfullscreen attribute.
//
// A boolean attribute that allows the embedded content to request fullscreen mode using the Fullscreen API. When present, content within the iframe can call document.requestFullscreen() and similar methods. Without this attribute, fullscreen requests from the iframe will be denied. Important for video players, games, and other immersive content that benefits from fullscreen presentation.
//
// Calling without an argument sets the attribute. Pass a bool to control presence conditionally.
func (e *Element) AllowFullscreen(conds ...bool) *Element {
	if len(conds) > 0 {
		e.allowfullscreen = conds[0]
	} else {
...
//
// Assigns a name to the embedded browsing context that can be used as a target for links, forms, and JavaScript operations. This name allows other elements to specifically target this iframe using the target attribute (e.g., target="frameName"). Useful for coordinated navigation between parent and child contexts, though modern web development often favors postMessage API for cross-frame communication.
...
//
// Controls how much referrer information is sent when the iframe loads its content and when navigation
// occurs within the iframe. This affects both the initial iframe load and subsequent navigation within the
// embedded context. Possible values: no-referrer, no-referrer-when-downgrade, origin,
...
//
// Applies security restrictions to the embedded content, creating a sandboxed environment that limits potentially dangerous operations. When present without a value, applies all restrictions. With space-separated values, selectively enables specific capabilities. Critical for embedding untrusted content safely.
// Possible values: allow-downloads, allow-forms, allow-modals, allow-orientation-lock, allow-pointer-lock, allow-popups, allow-popups-to-escape-sandbox, allow-presentation, allow-same-origin, allow-scripts, allow-storage-access-by-user-activation, allow-top-navigation, allow-top-navigation-by-user-activation
func (e *Element) Sandbox(value ...sandbox.Sandbox) *Element {
	if len(value) == 0 {
		return e
	}
	if len(e.sandbox) == 0 {
		for i, item := range value {
			if i > 0 {
				e.sandbox = append(e.sandbox, " "...)
			}
			e.sandbox = append(e.sandbox, item...)
		}
	} else {
		e.sandbox = append(e.sandbox, " "...)
		for i, item := range value {
...
//
// Contains inline HTML content to display within the iframe instead of loading from an external URL. When present, this attribute takes precedence over the src attribute. The content is treated as a complete HTML document within the iframe's sandboxed environment. Useful for embedding generated content, previews, or isolated HTML snippets without requiring separate files.
func (e *Element) SrcDoc(html string) *Element {
	e.SetAttribute("srcdoc", html)
	return e
}
...
// Credentialless sets the credentialless attribute.
//
// Creates a credentialless iframe that loads its content in a new, ephemeral context without access to
// the network, cookies, or storage data associated with its origin. Provides strong isolation for
// embedding untrusted third-party content.
//
// Calling without an argument sets the attribute. Pass a bool to control presence conditionally.
func (e *Element) Credentialless(conds ...bool) *Element {
	if len(conds) > 0 {
		e.credentialless = conds[0]
	} else {
...
func (e *Element) AttributeBuilder(buf *bytes.Buffer) {
	...
	if e.allowfullscreen {
		buf.Write(html5.AttrAllowFullscreen)
	}
	if e.credentialless {
		buf.Write(html5.AttrCredentialless)
	}
	if e.class != "" {
...
func (e *Element) RenderBuilder(buf *bytes.Buffer) {
	...
	buf.Write(html5.TagIframeClose)
}
