...
// Allow Specifies the permissions policy (Feature Policy) for the iframe, controlling which browser features the
...
// Sandbox Applies security restrictions to the embedded content, creating a sandboxed environment that limits potentially dangerous operations. When present without a value, applies all restrictions. With space-separated values, selectively enables specific capabilities. Critical for embedding untrusted content safely.
...
// Credentialless Creates a credentialless iframe that loads its content in a new, ephemeral context without access to
