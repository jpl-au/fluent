...
//
// The <button> element provides a control that can be activated with pointer
// or keyboard input. Its type selects submission, reset or an ordinary button
// action. Set type="button" for a control inside a form that must not submit it.
package button
...
//
// Specifies the action to be performed on an element being controlled by a control <button>, specified via
// the commandfor attribute. Possible values include: "show-modal", "close", "request-close", "show-popover",
// "hide-popover", "toggle-popover", or custom values prefixed with "--".
func (e *Element) Command(action string) *Element {
	e.SetAttribute("command", action)
	return e
}

// CommandFor sets the commandfor attribute.
//
// Turns a <button> element into a command button that controls an interactive element. Takes the ID
...
// the id of a <form> in the same document.
func (e *Element) Form(id string) *Element {
	e.SetAttribute("form", id)
	return e
}
...
//
// If the button is a submit button, this attribute is an author-defined name or standardised,
...
//
// The default behaviour of the button.
// Possible values: submit, reset, button.
func (e *Element) Type(value string) *Element {
	e.buttonType = node.EscapeAttribute(value)
	return e
}
...
func (e *Element) RenderBuilder(buf *bytes.Buffer) {
	...
	buf.Write(html5.TagButtonClose)
}
