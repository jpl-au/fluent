...
//
// The <img> HTML element embeds an image into the document, displaying graphics, photographs, illustrations,
// or visualizations. Supports responsive images via srcset and sizes, lazy loading for performance, and
// various decoding strategies. The alt attribute is essential for accessibility. Used for content images,
// logos, icons, diagrams, and any visual content integral to page meaning.
package img
...
//
// Controls the loading strategy for the image to optimize page performance. Lazy loading defers image loading until the image is about to enter the viewport, reducing initial page load time and bandwidth usage.
...
//
// Specifies the intended display size of the image at different viewport widths, working with srcset to enable responsive images. This attribute tells the browser how much space the image will occupy in various layouts, allowing it to select the most appropriate image from the srcset. Crucial for responsive design and bandwidth optimization. Format: '(max-width: 600px) 480px, 800px'.
...
//
// Controls Cross-Origin Resource Sharing (CORS) behavior when fetching the image from a different origin. This affects whether cookies and authentication are sent with the request, and whether the image can be used in certain contexts like canvas without tainting.
...
//
// Provides a hint about how the browser should prioritize image decoding relative to other tasks. This can help optimize user experience by controlling when images are processed and painted.
// Possible values: sync (decode synchronously for immediate presentation), async (decode asynchronously to avoid blocking other tasks), auto (let browser decide, default)
...
//
// Hints at the relative priority for fetching this image compared to other resources, helping browsers optimize resource loading order for better user experience. High priority images (like hero images) load first, while low priority images (like below-the-fold content) load later.
...
//
// Specifies that the browser should send an Attribution-Reporting-Eligible header with the image request.
