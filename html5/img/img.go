...
//
// The <img> element embeds an image with a text alternative supplied by alt.
// Srcset and sizes let the browser select a suitable image for its display size
// and pixel density. Width and height help reserve space before loading.
package img
...
// The browser can defer loading until the image is near the viewport.
...
//
// The image URL, which may be absolute, relative or a data URL. When srcset
// is also present, the browser may select a different candidate. Alt supplies
// the text alternative if the image is unavailable or cannot be perceived.
...
//
// Provides a text alternative for the image, including for screen readers
// and when the image cannot be displayed. Describe its purpose in context.
// For a decorative image that conveys no information, use an empty alt="".
...
//
// The image's intended width in CSS pixels, written as an integer without
// units. Together with height it supplies an aspect ratio before the resource
// loads. CSS can override the displayed dimensions.
...
//
// The image's intended height in CSS pixels, written as an integer without
// units. Together with width it supplies an aspect ratio before the resource
// loads. These dimensions need not match the file's pixel dimensions.
...
//
// Hints when to fetch the image. Lazy loading permits deferral until it is
// near the viewport, which can reduce initial data transfer. Eager requests
// normal loading without that deferral.
// Possible values: eager (load immediately, default), lazy (defer loading until needed)
...
//
// The expected rendered width used to select a srcset width candidate.
// For example, "(max-width: 600px) 100vw, 50vw" describes two layout sizes.
// It helps resource selection but does not set the image's CSS width.
...
//
// Comma-separated image candidates with width descriptors, such as
// "small.jpg 480w, large.jpg 960w", or density descriptors such as
// "normal.jpg 1x, retina.jpg 2x". Use sizes with width descriptors. For format
// fallback, use picture and source elements with type attributes.
...
//
// Controls Cross-Origin Resource Sharing (CORS) behaviour when fetching the image from a different origin. This affects whether cookies and authentication are sent with the request, and whether the image can be used in certain contexts like canvas without tainting.
// Possible values: anonymous (no credentials sent), use-credentials (send cookies and authentication)
...
//
// Hints how to coordinate image decoding with other rendering. Sync
// favours presenting the image with other content. Async permits that content
// to be presented without waiting. Auto leaves the choice to the browser.
func (e *Element) Decoding(hint decoding.Decoding) *Element {
	e.decoding = hint
	return e
}
...
//
// Hints the image's fetch priority relative to other images: high, low or
// auto. The browser still decides scheduling. High does not guarantee that
// this image loads before every other resource.
...
//
// Controls how much referrer information is sent when fetching the image, affecting privacy and security
...
//
// Indicates that the image is part of a server-side image map. When clicked inside a link, the coordinates
...
//
// Calling without an argument sets the attribute. Pass a bool to control presence conditionally.
func (e *Element) IsMap(conds ...bool) *Element {
	if len(conds) > 0 {
		e.ismap = conds[0]
	} else {
		e.ismap = true
...
//
// Associates the image with a client-side image map defined by a <map> element, enabling clickable regions
...
func (e *Element) UseMap(mapName string) *Element {
	e.usemap = node.EscapeAttribute(mapName)
	return e
}

// AttributionSrc sets the attributionsrc attribute.
//
// Requests an Attribution-Reporting-Eligible header with the image request.
// Enables privacy-preserving conversion measurement for the Attribution Reporting API. The value can be
// empty to use the image URL as the registration endpoint, or contain one or more space-separated URLs
// for specific attribution endpoints.
...
//
// Marks the image for observation by the PerformanceElementTiming API, enabling measurement of when the
// image is rendered. The value is a developer-defined label used to identify this element in performance
// entries. Useful for tracking Largest Contentful Paint (LCP) and other rendering performance metrics.
func (e *Element) ElementTiming(label string) *Element {
	e.SetAttribute("elementtiming", label)
	return e
}
...
func (e *Element) AttributeBuilder(buf *bytes.Buffer) {
	...
	if len(e.decoding) > 0 {
		buf.Write(html5.AttrDecoding)
		buf.Write(e.decoding)
		buf.Write(html5.MarkupQuote)
	}
