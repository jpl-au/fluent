...
// Text Creates a new textarea element with text content
...
// Static Creates a new textarea element with static text content.
// Example: textarea.Static("Hello World")
// Renders: <textarea>Hello World</textarea>
...
// RawText Creates a new textarea element with raw text content as unescaped HTML.
// Example: textarea.RawText("<em>Default</em> text")
// Renders: <textarea><em>Default</em> text</textarea>
...
// Textf Creates a new textarea element with formatted text content using text.Textf.
// Example: textarea.Textf("Enter your %s...", "comment")
// Renders: <textarea>Enter your comment...</textarea>
...
// RawTextf Creates a new textarea element with formatted raw text content as unescaped HTML using text.RawTextf.
// Example: textarea.RawTextf("<em>%s</em> text", "Default")
// Renders: <textarea><em>Default</em> text</textarea>
...
// Rows Defines the number of visible text lines (height) for the textarea control. This sets the initial visible
...
// Cols Specifies the visible width of the textarea control measured in average character widths. This determines
...
// AutoComplete Controls whether the browser should provide autocomplete suggestions based on previously entered values for
...
// AutoFocus A boolean attribute that automatically focuses this textarea when the page loads or when a dialog
// containing it is displayed. Only one form element per document should have autofocus to avoid conflicting
// focus behavior. Should be used thoughtfully as it can interfere with accessibility tools and user
// navigation expectations. Most effective for primary text input areas where immediate typing is expected.
func (e *element) AutoFocus() *element {
	e.autofocus = true
	return e
}

// Disabled A boolean attribute that disables the textarea, preventing user interaction and form submission of its
// value. When disabled, the element appears grayed out, cannot receive focus, and its value is not included
// in form submissions. Useful for conditional form logic where text input should be temporarily unavailable
...
// Form Associates the textarea with a form element in the document by referencing the form's ID attribute. This
...
// MaxLength Specifies the maximum number of characters (UTF-16 code units) that the user can enter into the textarea.
...
// MinLength Defines the minimum number of characters (UTF-16 code units) required for the textarea value to be
...
// Placeholder Provides hint text that appears inside the textarea when it's empty, disappearing when the user starts
...
// ReadOnly A boolean attribute that makes the textarea read-only, preventing user modification while still allowing the
// content to be selected, copied, and included in form submission. Unlike disabled elements, read-only
// textareas can receive focus and their values are submitted with forms. Useful for displaying pre-filled
// content that should not be changed, such as terms of service, calculated values, or locked template text.
func (e *element) ReadOnly() *element {
	e.readOnly = true
	return e
}

// Required A boolean attribute that makes this textarea mandatory for form submission. Browsers will prevent form
// submission and display validation messages if the textarea is empty or contains only whitespace. Works
// with browser built-in form validation and provides automatic validation feedback to users. Essential for
// ensuring required information is collected, such as mandatory comments, descriptions, or feedback.
func (e *element) Required() *element {
	e.required = true
	return e
}

// SpellCheck Controls whether the browser should check spelling and grammar in the textarea content. Accepts string values:
...
// Wrap Controls how the textarea wraps text for form submission, affecting whether line breaks from visual
