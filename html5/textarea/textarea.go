...
//
// Specifies the name of the textarea element for form submission and programmatic access. This name is used
// to identify the textarea's value in form data when submitted to the server. The name appears as the key
// in form submissions. Essential for server-side form processing and JavaScript form manipulation via the
// form.elements API.
...
//
// Defines the number of visible text lines (height) for the textarea control. This sets the initial visible
// height of the textarea in terms of text lines. Users can typically resize the textarea if the browser and
...
//
// Specifies the visible width of the textarea control measured in average character widths. This determines
// how many characters can typically fit horizontally before wrapping to the next line. While users can often
// resize the textarea, this sets the initial width. The actual pixel width depends on the font family and
...
//
// Controls whether the browser should provide autocomplete suggestions based on previously entered values for
// this textarea. This can enhance user experience by suggesting commonly used text, but may not be
// appropriate for sensitive or unique content. Common values include 'on', 'off', or specific tokens like
// 'street-address', 'organization', or 'current-password' for semantic autocompletion.
func (e *Element) AutoComplete(autocomplete autocomplete.AutoComplete) *Element {
...
//
// Disables the textarea, preventing user interaction. The element appears greyed out, cannot receive focus,
// and its value is not included in form submissions.
//
...
//
// Associates the textarea with a form element in the document by referencing the form's ID attribute. This
// allows the textarea to participate in form submission and validation even when not physically nested
// within the form element. The value must match the ID of a <form> element in the same document. Enables
// flexible form layouts where form controls can be placed anywhere in the document.
...
//
// Specifies the maximum number of characters (UTF-16 code units) that the user can enter into the textarea.
// Browsers prevent additional input once this limit is reached and may display a character counter. This
// constraint is enforced during user input and form validation. Essential for preventing overly long
// submissions, maintaining database field limits, and providing clear user guidance about content length
// expectations.
func (e *Element) MaxLength(length int) *Element {
	e.SetAttribute("maxlength", strconv.Itoa(length))
	return e
}

// MinLength sets the minlength attribute.
//
// Defines the minimum number of characters (UTF-16 code units) required for the textarea value to be
// considered valid. Form submission will be prevented and validation messages displayed if the content is
// shorter than this minimum. Works with browser built-in form validation to ensure adequate content length.
// Useful for ensuring meaningful responses in comments, reviews, or other text areas where brief responses
// are insufficient.
...
//
// Provides hint text that appears inside the textarea when it's empty, disappearing when the user starts
// typing. This should be instructional text that helps users understand what kind of content is expected,
// not a substitute for proper labeling. The placeholder text should be concise and descriptive, such as
// 'Enter your feedback here...' or 'Describe the issue in detail'. Does not replace the need for proper
...
//
// Makes the textarea read-only, preventing user modification while still allowing the content to be
// selected, copied, and submitted with forms. Unlike disabled, read-only textareas can receive focus.
//
// Calling without an argument sets the attribute. Pass a bool to control presence conditionally.
func (e *Element) ReadOnly(conds ...bool) *Element {
	if len(conds) > 0 {
		e.readOnly = conds[0]
	} else {
		e.readOnly = true
...
//
// Makes this textarea mandatory for form submission. Browsers prevent submission and display validation
// messages if the textarea is empty or contains only whitespace.
//
// Calling without an argument sets the attribute. Pass a bool to control presence conditionally.
...
//
// Controls whether the browser should check spelling and grammar in the textarea content. Accepts string values:
// "true" (enable spell checking), "false" (disable spell checking), or "default" (use parent element's setting).
// When enabled, browsers typically underline misspelled words and may offer correction suggestions.
// Should be disabled for content where spell checking is not appropriate, such as code snippets, technical
// terms, usernames, or non-dictionary words. Improves user experience for natural language text input.
func (e *Element) SpellCheck(spellcheck spellcheck.Spellcheck) *Element {
...
//
// Controls how the textarea wraps text for form submission, affecting whether line breaks from visual
// wrapping are included in the submitted data. This only affects form submission behavior, not the visual
// display of text in the textarea.
// Possible values: soft (visual wrapping only, no line breaks in submitted data), hard (include line breaks
// in submitted data where visual wrapping occurs), off (no wrapping)
func (e *Element) Wrap(wrap string) *Element {
	e.SetAttribute("wrap", wrap)
	return e
}

// DirName sets the dirname attribute.
//
// Enables automatic submission of the text directionality (left-to-right or right-to-left) of the textarea's
...
func (e *Element) DirName(name string) *Element {
	e.dirname = node.EscapeAttribute(name)
	return e
}
...
func (e *Element) AttributeBuilder(buf *bytes.Buffer) {
	...
	if len(e.spellcheck) > 0 {
		buf.Write(html5.AttrSpellCheck)
		buf.Write(e.spellcheck)
		buf.Write(html5.MarkupQuote)
	}
...
func (e *Element) RenderBuilder(buf *bytes.Buffer) {
	...
	buf.Write(html5.TagTextareaClose)
}
