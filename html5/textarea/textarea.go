...
//
// The name used for the textarea's submitted value and named access through
// the form. It does not provide the visible label. Use a label element.
...
//
// The requested visible height, measured in text lines. It controls the initial
// size. It does not limit the number of lines in the value. CSS and user resizing
// can change the displayed height.
...
//
// The requested visible width, measured in average character widths. It does
// not limit text length. The rendered width depends on the font and CSS.
...
//
// Requests autocomplete suggestions based on previously entered values for
...
//
// The id of the form that owns the textarea. It must identify a form in the
// same document, even when the textarea is placed outside the form.
...
//
// Limits user-entered text length in UTF-16 code units. Some characters, such
// as many emoji, count as two units. Browsers may prevent input beyond the limit.
// This is a client-side constraint. It does not set a byte limit for storage.
...
//
// The minimum permitted length of a non-empty user-entered value, measured in
// UTF-16 code units. Empty content is still permitted unless required is set.
...
//
// Provides hint text inside an empty textarea. It disappears when the user
// starts typing. Describe the expected content, for example 'Describe the
// issue in detail'. Provide a separate label that remains visible during input.
...
//
// Requires a non-empty value during form validation. Whitespace is part of
// the value and does not by itself make the textarea empty.
//
// Calling without an argument sets the attribute. Pass a bool to control presence conditionally.
...
//
// Controls whether the textarea content is eligible for spelling and grammar checking. Accepts string values:
...
//
// Controls line wrapping and its effect on submission. Soft does not insert
// extra line breaks into the submitted value. Hard permits additional line
// breaks based on the control's width and requires cols. Off is a non-standard
// value that disables visual wrapping.
