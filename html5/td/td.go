...
//
// The <td>: The Table Data Cell element defines a cell of a table that contains data. It participates in the
// table model.
package td
...
// Element represents the <td> HTML element.
type Element struct {
...
// Text creates a new td element with text content. Uses text.Text which HTML-escapes the output.
// Example: td.Text("Alice")
// Renders: <td>Alice</td>
...
// Static creates a new td element with static text content. Uses text.Static which is not HTML-escaped and is JIT-optimisable.
// Example: td.Static("Total")
// Renders: <td>Total</td>
...
// RawText creates a new td element with raw text content. Uses text.RawText which is not HTML-escaped.
// Example: td.RawText("<a href=\"/user/1\">Alice</a>")
// Renders: <td><a href="/user/1">Alice</a></td>
...
// Textf creates a new td element with formatted text content. Uses text.Textf which HTML-escapes the output.
// Example: td.Textf("$%d", amount)
// Renders: <td>$100</td>
...
// RawTextf creates a new td element with formatted raw text content. Uses text.RawTextf which is not HTML-escaped.
// Example: td.RawTextf("<a href=\"/user/%d\">%s</a>", id, name)
// Renders: <td><a href="/user/42">Mary</a></td>
...
//
// Specifies how many columns this cell should span horizontally, allowing the cell to extend across multiple
// columns in the table. The value must be a positive integer. A value of 1 (default) means the cell
...
//
// Specifies how many rows this cell should span vertically, allowing the cell to extend across multiple rows
// in the table. The value must be a positive integer. A value of 1 (default) means the cell occupies one
// row, while higher values create merged cells that span multiple rows. Useful for creating complex table
// layouts with grouped data or merged cells that span across row boundaries.
...
//
// Contains a space-separated list of ID values that correspond to the id attributes of <th> (table header)
// elements that apply to this data cell. This creates explicit associations between data cells and their
// related headers, which is crucial for accessibility as screen readers use these relationships to provide
// context to users navigating table data. Essential for complex tables where header relationships aren't
// obvious from structure alone.
func (e *Element) Headers(ids string) *Element {
	e.SetAttribute("headers", ids)
	return e
}
...
func (e *Element) AttributeBuilder(buf *bytes.Buffer) {
	...
	if e.rowSpan != nil {
		buf.Write(html5.AttrRowSpan)
		buf.Write(strconv.AppendInt(nil, int64(*e.rowSpan), 10))
		buf.Write(html5.MarkupQuote)
	}
	if e.class != "" {
