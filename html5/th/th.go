...
//
// The <th> HTML element defines a cell as header of a group of table cells. The exact nature of this group is
...
//
// Specifies how many columns this header cell should span horizontally, allowing the cell to extend across
...
//
// Specifies how many rows this header cell should span vertically, allowing the cell to extend across
// multiple rows in the table. The value must be a positive integer. A value of 1 (default) means the cell
// occupies one row, while higher values create merged header cells that span multiple rows. Useful for
// creating multi-level table headers with hierarchical groupings.
...
//
// Contains a space-separated list of ID values that correspond to the id attributes of other <th> elements
// that also apply to this header cell. This creates hierarchical relationships between header cells in
// complex tables with multi-level headers. Essential for accessibility in tables with nested or grouped
// headers, helping screen readers understand the complete header context.
...
//
// Defines which cells this header cell applies to, establishing the semantic relationship between the header
// and data cells. This is crucial for table accessibility as it helps screen readers associate header
// information with the appropriate data cells when navigating tables.
// Possible values: row (header applies to the rest of the row), col (header applies to the rest of the
// column), rowgroup (header applies to all cells in the row group), colgroup (header applies to all cells
// in the column group)
func (e *Element) Scope(scope string) *Element {
	e.scope = node.EscapeAttribute(scope)
	return e
}

// Abbr sets the abbr attribute.
//
// Provides a short abbreviated description of the header cell's content. Screen readers may present this
...
func (e *Element) Abbr(abbreviation string) *Element {
	e.abbr = node.EscapeAttribute(abbreviation)
	return e
}
