...
//
// The <link> element declares a relationship to an external resource. Rel
// selects the relationship, such as stylesheet, icon or preload, and href
// identifies the resource. Attributes such as media and as further qualify
...
// The browser uses this relationship to interpret and handle the resource.
...
//
// The URL of the linked resource. Rel determines its role, such as a stylesheet,
// icon or preload request. Relative URLs are resolved against the document's
// base URL.
//
// Fluent filters this URL at render time: http, https, mailto, tel, sms and relative URLs
// render as given; any other scheme (javascript:, data:, file:, a custom protocol) renders as the
...
//
// Identifies the request destination for preloading. The browser uses it to
// prioritise the request and apply the relevant content security policy.
// Set it to match the resource's intended consumer.
...
//
// Controls Cross-Origin Resource Sharing (CORS) behaviour when fetching the linked resource from a different
// origin. Particularly important for resources used in contexts where CORS matters, such as
...
//
// For a stylesheet link, prevents the stylesheet from being applied. When
// present in the initial markup it can also prevent the resource from being
// loaded until enabled. It is not a preload mechanism.
//
...
//
// Subresource Integrity metadata used to check a fetched resource against an
// expected hash, such as "sha384-...". The browser rejects a resource that
// fails verification. Cross-origin integrity checks also require a suitable
// CORS response.
...
//
// A media query describing when the linked resource applies, such as "print"
// or "(prefers-color-scheme: dark)". For a stylesheet, a non-matching query
// prevents application but does not guarantee that the file is not fetched.
...
//
// A media-type hint for the linked resource, such as text/css or image/png.
// The browser can use it to avoid unsupported resources. Its effect depends
// on the relationship specified by rel.
...
//
// Requests that the browser block the specified operations until the resource is loaded.
...
//
// Specifies the colour for rel="mask-icon" (Safari pinned tab icons).
