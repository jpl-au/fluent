...
//
// The <link> HTML element specifies relationships between the current document and external resources.
// Primarily used for linking stylesheets, establishing site icons (favicons and app icons), and resource
// preloading/prefetching. Appears in the document head. Essential for applying styles, setting page icons,
// optimising resource loading, and establishing document relationships.
package link
...
//
// Defines the relationship between the current document and the linked resource. This attribute determines how the browser should handle and interpret the linked resource.
...
//
// Specifies the type of content being loaded when using rel="preload" or rel="prefetch", helping the browser prioritize and handle the resource appropriately. This attribute is required for preload links to ensure proper resource prioritization.
// Possible values: audio, document, embed, fetch, font, image, object, script, style, track, video, worker
func (e *Element) As(as as.As) *Element {
	e.as = as
	return e
}
...
//
// Controls Cross-Origin Resource Sharing (CORS) behavior when fetching the linked resource from a different
// origin. Particularly important for resources that will be used in contexts where CORS matters, such as
// fonts or scripts. Possible values: anonymous (no credentials), use-credentials (send cookies and
// authentication)
...
//
// For stylesheet links only, this boolean attribute controls whether the stylesheet should be loaded and applied to the document. When true, the stylesheet is loaded but not applied, allowing for programmatic control of stylesheet activation. This enables dynamic theme switching, conditional styling, and performance optimizations where stylesheets can be loaded but activated only when needed.
//
// Calling without an argument sets the attribute. Pass a bool to control presence conditionally.
func (e *Element) Disabled(conds ...bool) *Element {
	if len(conds) > 0 {
		e.disabled = conds[0]
	} else {
		e.disabled = true
...
//
// Specifies the primary language of the linked resource using a valid BCP 47 language tag (e.g., 'en', 'en-US', 'fr-CA'). This information helps browsers, search engines, and assistive technologies understand the language context of the linked content. Particularly useful for alternate versions of documents in different languages, helping users and tools select the appropriate version.
...
//
// Contains a cryptographic hash that the browser uses to verify the integrity of the fetched resource, ensuring it hasn't been tampered with during transmission. This is a critical security feature for external resources, especially those loaded from CDNs. Format: 'sha384-hash' or 'sha512-hash'. Essential for security-conscious applications and preventing supply chain attacks.
func (e *Element) Integrity(hash string) *Element {
	e.SetAttribute("integrity", hash)
	return e
}

// Media sets the media attribute.
//
// Specifies the media types and conditions for which the linked resource is intended, using CSS media query syntax. This allows conditional loading of resources based on device characteristics, screen size, orientation, and other factors. Examples: 'screen', 'print', 'screen and (max-width: 600px)', '(prefers-color-scheme: dark)'. Essential for responsive design and optimized resource loading.
func (e *Element) Media(query string) *Element {
	e.SetAttribute("media", query)
	return e
}

// ReferrerPolicy sets the referrerpolicy attribute.
//
// Controls how much referrer information is sent when fetching the linked resource, affecting privacy and security when loading external assets.
...
//
// Specifies the dimensions of icons when rel="icon" is used, helping browsers select the most appropriate icon size for different contexts (favicon, app icons, etc.). Format: 'widthxheight' or multiple sizes separated by spaces like '16x16 32x32 48x48'. Modern browsers use this information to choose optimal icons for bookmarks, tabs, home screen shortcuts, and other UI elements.
func (e *Element) Sizes(sizes ...sizes.Size) *Element {
	if len(sizes) == 0 {
		return e
	}
	if len(e.sizes) == 0 {
		for i, item := range sizes {
			if i > 0 {
				e.sizes = append(e.sizes, " "...)
			}
			e.sizes = append(e.sizes, item...)
		}
	} else {
		e.sizes = append(e.sizes, " "...)
		for i, item := range sizes {
...
//
// Provides a human-readable title for the linked resource, particularly important for alternate stylesheets where it appears in browser stylesheet selection menus. For alternate stylesheets (rel="alternate stylesheet"), this creates user-selectable themes. For regular stylesheets, it provides descriptive information. Users can switch between titled alternate stylesheets through browser interfaces.
...
//
// Specifies the MIME type of the linked resource, helping browsers understand how to handle the content before fetching it. This can improve performance by avoiding unnecessary downloads of unsupported formats. Examples: 'text/css' for stylesheets, 'image/x-icon' for favicons, 'application/rss+xml' for RSS feeds. While often optional due to file extensions, it provides explicit type information for better resource handling.
func (e *Element) Type(mimeType string) *Element {
	e.mimeType = node.EscapeAttribute(mimeType)
	return e
}

// FetchPriority sets the fetchpriority attribute.
//
// Hints at the relative priority for fetching this resource compared to other resources.
...
//
// Indicates that certain operations should be blocked until the resource is loaded.
...
//
// For rel="preload" and as="image" only. Specifies image display sizes for responsive preloading.
// Works with imagesrcset to help browser select appropriate image.
func (e *Element) ImageSizes(sizes string) *Element {
	e.imagesizes = node.EscapeAttribute(sizes)
	return e
}

// Color sets the color attribute.
//
// Specifies color for rel="mask-icon" (Safari pinned tab icons).
// Accepts any valid CSS color value.
func (e *Element) Color(color string) *Element {
	e.color = node.EscapeAttribute(color)
	return e
}
...
func (e *Element) AttributeBuilder(buf *bytes.Buffer) {
	...
	if len(e.as) > 0 {
		buf.Write(html5.AttrAs)
		buf.Write(e.as)
		buf.Write(html5.MarkupQuote)
	}
	if len(e.sizes) > 0 {
		buf.Write(html5.AttrSizes)
		buf.Write(e.sizes)
		buf.Write(html5.MarkupQuote)
	}
