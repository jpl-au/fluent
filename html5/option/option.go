...
// element. As such, <option> can represent menu items in popups and other lists of items in an HTML document.
package option
...
//
// Specifies the value submitted to the server when this option is selected
...
//
// Provides alternative text for the option that can be displayed instead of the option's text content
func (e *Element) Label(label string) *Element {
	e.label = node.EscapeAttribute(label)
	return e
}

// Selected sets the selected attribute.
//
// Marks this option as pre-selected when the page loads.
//
// Calling without an argument sets the attribute. Pass a bool to control presence conditionally.
func (e *Element) Selected(conds ...bool) *Element {
	if len(conds) > 0 {
		e.selected = conds[0]
	} else {
		e.selected = true
...
func (e *Element) RenderBuilder(buf *bytes.Buffer) {
	...
	buf.Write(html5.TagOptionClose)
}
