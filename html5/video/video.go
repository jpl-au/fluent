...
// Autoplay Starts video playback automatically. Many browsers restrict autoplay with sound - typically only works
...
// Controls Displays built-in video controls (play/pause, progress bar, volume, fullscreen). Highly recommended
...
// Loop Restarts the video from the beginning when it reaches the end. Useful for background videos and
// demonstrations.
//
// Calling without an argument sets the attribute. Pass a bool to control presence conditionally.
...
// Muted Mutes the video's audio by default. Often necessary for autoplay as many browsers block autoplaying
// videos with audio.
//
// Calling without an argument sets the attribute. Pass a bool to control presence conditionally.
...
// Loading Controls how the browser loads the video element. Use lazy loading to defer loading until the element
...
// CrossOrigin Controls CORS behaviour when fetching the video. Required when the video will be used with canvas or WebGL
