...
//
// The <video> element embeds a video player. Use src for a single resource or
// source children for alternatives, and track children for timed text. Controls
// requests browser playback controls. Poster supplies an image before playback.
package video
...
// PreloadAuto creates a video element that allows the browser to preload the
// entire resource. This hint does not guarantee a complete download.
...
// PreloadMetadata creates a video element that requests metadata, such as
// duration, without asking the browser to preload the entire resource.
...
// PreloadNone creates a video element that requests no advance fetching.
// The browser may still fetch data when playback or autoplay requires it.
...
//
// The URL of a single video resource. Use source children instead when
// providing alternative encodings for the browser to choose between.
...
//
// Starts video playback automatically. Many browsers restrict autoplay with sound - typically only works
// for muted videos or when the user has previously interacted with the site.
//
// Calling without an argument sets the attribute. Pass a bool to control presence conditionally.
func (e *Element) Autoplay(conds ...bool) *Element {
	if len(conds) > 0 {
		e.autoplay = conds[0]
	} else {
		e.autoplay = true
...
//
// Displays built-in video controls (play/pause, progress bar, volume, fullscreen). Highly recommended
// for accessibility unless providing custom JavaScript controls.
//
// Calling without an argument sets the attribute. Pass a bool to control presence conditionally.
...
//
// Sets the height of the video's display area in CSS pixels. Percentages and
// other relative units are not allowed. This defines the intrinsic dimensions of the video element and helps
...
//
// Sets the width of the video's display area in CSS pixels. Percentages and
// other relative units are not allowed. Combined with height, this establishes the aspect ratio and display size
...
//
// Restarts the video from the beginning when it reaches the end. Useful for background videos and
// demonstrations.
//
// Calling without an argument sets the attribute. Pass a bool to control presence conditionally.
func (e *Element) Loop(conds ...bool) *Element {
	if len(conds) > 0 {
		e.loop = conds[0]
	} else {
		e.loop = true
...
//
// Mutes the video's audio by default. Often necessary for autoplay as many browsers block autoplaying
// videos with audio.
//
// Calling without an argument sets the attribute. Pass a bool to control presence conditionally.
func (e *Element) Muted(conds ...bool) *Element {
	if len(conds) > 0 {
		e.muted = conds[0]
	} else {
		e.muted = true
...
//
// The URL of an image representing the video before video frames are shown.
// It occupies the video's display area. It is separate from the video resource
// and does not provide a text alternative.
func (e *Element) Poster(url string) *Element {
	e.poster = node.EscapeAttribute(url)
	return e
}

// Preload sets the preload attribute.
//
// Hints how much video to fetch before playback: none, metadata or auto.
// The browser may ignore the hint based on its policy or current conditions.
// Autoplay can require fetching regardless of the preload value.
...
//
// Controls how the browser loads the video element. Use lazy loading to defer loading until the element
// is near the viewport, improving initial page load performance.
// Possible values: eager (load immediately, default), lazy (defer loading until near viewport)
...
//
// Controls CORS behaviour when fetching the video. Required when using cross-origin video with canvas or WebGL
...
//
// Requests omission of selected browser controls, using nodownload,
// nofullscreen or noremoteplayback. Supported options depend on the browser.
// Hiding a control does not restrict access to the media resource.
...
// DisablePictureInPicture sets the disablepictureinpicture attribute.
//
// Disables Picture-in-Picture mode for this video and removes the option from context menus and browser UI.
// Picture-in-Picture allows videos to play in a floating window while users interact with other content.
//
// Calling without an argument sets the attribute. Pass a bool to control presence conditionally.
func (e *Element) DisablePictureInPicture(conds ...bool) *Element {
	if len(conds) > 0 {
		e.disablepictureinpicture = conds[0]
	} else {
...
// DisableRemotePlayback sets the disableremoteplayback attribute.
//
// Disables remote playback on connected devices such as Chromecast, AirPlay, and other casting technologies.
// Use when playback is intended to remain on the current device.
//
// Calling without an argument sets the attribute. Pass a bool to control presence conditionally.
func (e *Element) DisableRemotePlayback(conds ...bool) *Element {
	if len(conds) > 0 {
		e.disableremoteplayback = conds[0]
	} else {
...
//
// Requests inline playback within the element's area on the page.
// This does not itself enable autoplay or prevent the user from choosing
// fullscreen playback.
//
// Calling without an argument sets the attribute. Pass a bool to control presence conditionally.
func (e *Element) PlaysInline(conds ...bool) *Element {
	if len(conds) > 0 {
		e.playsinline = conds[0]
	} else {
		e.playsinline = true
	}
	return e
...
func (e *Element) AttributeBuilder(buf *bytes.Buffer) {
	...
	if e.loop {
		buf.Write(html5.AttrLoop)
	}
	if e.muted {
		buf.Write(html5.AttrMuted)
	}
	if e.poster != "" {
		buf.Write(html5.AttrPoster)
