...
// Text Creates a new video element with text content (fallback text)
// Example: video.Text("Your browser doesn't support video")
// Renders: <video>Your browser doesn&#39;t support video</video>
...
// Static Creates a new video element with static text content.
// Example: video.Static("Hello World")
// Renders: <video>Hello World</video>
...
// RawText Creates a new video element with raw text content as unescaped HTML.
// Example: video.RawText("Your browser <em>doesn't</em> support video")
// Renders: <video>Your browser <em>doesn't</em> support video</video>
...
// Textf Creates a new video element with formatted text content using text.Textf.
// Example: video.Textf("Your browser %s support video", "doesn't")
// Renders: <video>Your browser doesn&#39;t support video</video>
...
// RawTextf Creates a new video element with formatted raw text content as unescaped HTML using text.RawTextf.
// Example: video.RawTextf("Your browser <em>%s</em> support video", "doesn't")
// Renders: <video>Your browser <em>doesn't</em> support video</video>
...
// Renders: <video src="movie.mp4"></video>
...
// Autoplay A boolean attribute that causes the video to start playing automatically as soon as it can without stopping to
// finish loading. However, many browsers restrict autoplay, especially with sound, to prevent unwanted media
// playback. Autoplay typically only works for muted videos or when the user has previously interacted with the
// site. Should be used sparingly and considerately for user experience.
func (e *element) Autoplay() *element {
	e.autoplay = true
	return e
}

// Controls A boolean attribute that instructs the browser to display built-in video controls such as play/pause button,
// progress bar, volume control, and fullscreen toggle. When present, users can interact with the video using
// standard media controls. Essential for user experience unless you're providing custom JavaScript-based
// controls. Highly recommended for accessibility and usability.
func (e *element) Controls() *element {
	e.controls = true
	return e
}

// Height Sets the height of the video's display area in CSS pixels. Only absolute pixel values are allowed, not
...
// Width Sets the width of the video's display area in CSS pixels. Only absolute pixel values are allowed, not
...
// Loop A boolean attribute that causes the video to automatically restart from the beginning when it reaches the end,
// creating a continuous playback loop. Useful for background videos, demonstrations, or artistic content where
// seamless repetition is desired. Should be used thoughtfully as looping videos can be distracting or consume
// unnecessary bandwidth and battery power.
func (e *element) Loop() *element {
	e.loop = true
	return e
}

// Muted A boolean attribute that mutes the video's audio by default. When present, the video starts playing without
// sound, though users can unmute it through the controls. This is often necessary for autoplay functionality,
// as many browsers block autoplaying videos with audio. Essential for background videos and helps comply with
// browser autoplay policies while maintaining user control over audio.
func (e *element) Muted() *element {
	e.muted = true
	return e
}

// Poster Specifies the URL of an image to display as a placeholder before the video loads or while the user hasn't
...
// Preload Provides a hint to the browser about how much of the video should be preloaded before the user requests
