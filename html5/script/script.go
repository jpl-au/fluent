...
// Text Creates a new script element with inline JavaScript code
...
// Static Creates a new script element with static text content
...
// RawText Creates a new script element with raw inline JavaScript code as unescaped HTML
...
// Textf Creates a new script element with formatted text content using text.Textf
// Example: script.Textf("console.log('%s');", "Hello")
// Renders: <script>console.log(&#39;Hello&#39;);</script>
...
// RawTextf Creates a new script element with formatted raw text content as unescaped HTML using text.RawTextf
// Example: script.RawTextf("var %s = %d;", "x", 1)
// Renders: <script>var x = 1;</script>
...
// Type Specifies the MIME type or language of the script content
func (e *element) Type(mime string) *element {
	e.scriptType = mime
	return e
}

// Async A boolean attribute that causes the script to be downloaded in parallel to parsing the document
func (e *element) Async() *element {
	e.async = true
	return e
}

// CrossOrigin Controls Cross-Origin Resource Sharing (CORS) behavior when fetching the script from a different origin
func (e *element) CrossOrigin(value crossorigin.CrossOrigin) *element {
	e.crossorigin = value
	return e
}

// Defer A boolean attribute that defers script execution until after the HTML document has been completely parsed
func (e *element) Defer() *element {
	e.deferred = true
	return e
}

// Integrity Contains a cryptographic hash that browsers use to verify the integrity of the fetched script
func (e *element) Integrity(hash string) *element {
	e.integrity = hash
	return e
}

// NoModule A boolean attribute that prevents the script from executing in browsers that support ES6 modules
func (e *element) NoModule() *element {
	e.noModule = true
	return e
}

// Nonce A cryptographic nonce that allows specific inline scripts to execute despite restrictive CSP policies
