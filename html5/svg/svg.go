...
// New Creates a new svg element with optional child nodes.
// Example: svg.New()
// Renders: <svg></svg>
...
// Text Creates a new svg element with text content.
// Example: svg.Text("SVG content")
// Renders: <svg>SVG content</svg>
...
// Static Creates a new svg element with static text content.
// Example: svg.Static("Hello World")
// Renders: <svg>Hello World</svg>
...
// RawText Creates a new svg element with raw text content as unescaped HTML.
// Example: svg.RawText("<text>SVG</text> content")
// Renders: <svg><text>SVG</text> content</svg>
...
// Textf Creates a new svg element with formatted text content using text.Textf.
// Example: svg.Textf("%s content", "SVG")
// Renders: <svg>SVG content</svg>
...
// RawTextf Creates a new svg element with formatted raw text content as unescaped HTML using text.RawTextf.
// Example: svg.RawTextf("<text>%s</text> content", "SVG")
// Renders: <svg><text>SVG</text> content</svg>
...
// Width Specifies the displayed width of the SVG viewport (the visible area), not the width of its internal
// coordinate system. This can be specified in various units like pixels, percentages, or other CSS units.
// The viewport width determines how much space the SVG occupies in the document layout, while the viewBox
// attribute controls the internal coordinate system and scaling of the SVG content.
func (e *element) Width(value string) *element {
	e.width = value
	return e
}

// Height Specifies the displayed height of the SVG viewport (the visible area), not the height of its internal
// coordinate system. This can be specified in various units like pixels, percentages, or other CSS units.
// Combined with width, this defines the aspect ratio and size of the SVG element in the document layout,
// while the viewBox controls the internal coordinate system and content scaling.
func (e *element) Height(value string) *element {
	e.height = value
	return e
}

// ViewBox Defines the internal coordinate system and visible area of the SVG content using four space-separated
// numbers: 'min-x min-y width height'. This establishes the coordinate system for all child elements and
// controls scaling and cropping. For example, '0 0 100 100' creates a 100x100 coordinate system starting at
// origin (0,0). Essential for creating scalable, responsive SVG graphics that maintain proportions across
...
// PreserveAspectRatio Controls how the SVG content should be scaled and positioned when the viewBox aspect ratio differs from the
// viewport aspect ratio. This attribute determines alignment and scaling behavior to prevent distortion.
// Common values include 'xMidYMid meet' (center and scale uniformly), 'none' (stretch to fill), or
// 'xMinYMin slice' (align to top-left and crop if needed). Essential for responsive SVG design.
func (e *element) PreserveAspectRatio(value string) *element {
	e.SetAttribute("preserveaspectratio", value)
	return e
}

// Xmlns Specifies the XML namespace for SVG elements, typically set to 'http://www.w3.org/2000/svg'. This namespace
// declaration tells the browser how to interpret SVG elements and is required for proper SVG rendering in XML
// contexts. While often optional in HTML5, it's good practice to include it for compatibility and explicit
// namespace declaration, especially when SVG is embedded or used in XML documents.
