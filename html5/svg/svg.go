...
//
// The <svg> HTML element is a container that defines a new coordinate system and viewport. It is used as the
// outermost element of SVG documents, but it can also be used to embed an SVG fragment inside an SVG or HTML
// document.
package svg
...
// element represents the <svg> HTML element
type element struct {
...
//
// Specifies the displayed width of the SVG viewport. Can be in pixels, percentages, or other CSS units.
// The viewport width determines how much space the SVG occupies in the document layout, while the viewBox
// attribute controls the internal coordinate system.
...
//
// Specifies the displayed height of the SVG viewport. Can be in pixels, percentages, or other CSS units.
// Combined with width, defines the aspect ratio and size of the SVG in the document layout.
...
//
// Defines the internal coordinate system using four space-separated numbers: 'min-x min-y width height'.
// Establishes the coordinate system for all child elements and controls scaling. For example,
// '0 0 100 100' creates a 100x100 coordinate system starting at origin (0,0). Essential for creating
// scalable, responsive SVG graphics.
func (e *element) ViewBox(value string) *element {
	e.SetAttribute("viewbox", value)
	return e
}

// PreserveAspectRatio sets the preserveaspectratio attribute.
//
// Controls how the SVG content scales when the viewBox aspect ratio differs from the viewport.
// Common values: 'xMidYMid meet' (centre and scale uniformly), 'none' (stretch to fill),
// 'xMinYMin slice' (align to top-left and crop if needed).
func (e *element) PreserveAspectRatio(value string) *element {
	e.SetAttribute("preserveaspectratio", value)
	return e
}

// Xmlns sets the xmlns attribute.
//
// Specifies the XML namespace for SVG elements, typically 'http://www.w3.org/2000/svg'. Required for
// proper SVG rendering in XML contexts. Optional in HTML5 but good practice for compatibility.
func (e *element) Xmlns(namespace string) *element {
