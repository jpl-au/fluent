...
//
// The <area> HTML element defines clickable regions within an image map. Image maps allow geometric areas on an
// image to be associated with hyperlinks, enabling interactive images with multiple destinations. This element must
// be used within a <map> element and supports rectangular, circular, polygonal, and default shapes for defining
// clickable regions.
package area
...
//
// The shape of the area. The shape determines how the coords attribute is interpreted.
// Possible values: rect (rectangles), circle (circles), poly (polygons), default (entire image).
func (e *Element) Shape(shape shape.Shape) *Element {
	e.shape = shape
	return e
}
...
//
// The coordinates of the area, given as a comma-separated list of numbers. The meaning depends on the shape:
// - rect: x1,y1,x2,y2 (top-left and bottom-right corners)
// - circle: x,y,radius (centre coordinates and radius)
...
//
// Alternative text for the area when the image cannot be displayed. Describe the purpose of the
...
//
// Indicates that the hyperlink is to be used for downloading a resource. The value specifies the suggested filename.
...
//
// A space-separated list of URLs. When the area is activated, the browser sends POST requests with the body
...
//
// The relationship between the current document and the linked document. Valid on area elements, this attribute
// defines the relationship as a space-separated list of link types. Common values include 'alternate', 'bookmark',
// 'help', 'license', 'next', 'nofollow', 'noopener', 'noreferrer', 'prev', 'search', and 'tag'.
...
//
// Where to display the linked URL, as the name for a browsing context (a tab, window, or <iframe>). Possible
...
// but is useful to applications, accessibility tools, and custom JavaScript handling. Use a
...
func (e *Element) AttributeBuilder(buf *bytes.Buffer) {
	...
	if e.coords != "" {
		buf.Write(html5.AttrCoords)
		buf.WriteString(e.coords)
		buf.Write(html5.MarkupQuote)
	}
	if e.href != "" {
