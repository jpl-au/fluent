...
// Coords The coordinates of the area, given as a comma-separated list of numbers. The meaning depends on the shape:
...
// Alt Alternative text for the area when the image cannot be displayed. This text should describe the purpose of the
...
// Download Indicates that the hyperlink is to be used for downloading a resource. The value specifies the suggested filename.
...
// HrefLang Hints at the human language of the linked URL. No built-in functionality. Allowed values are determined by BCP47.
// Use this attribute only if the href attribute is present.
...
// Media Specifies what media/device the linked document is optimised for. Used only when href is present.
// Example values include 'screen', 'print', 'screen and (max-width: 600px)'.
func (e *element) Media(mediaquery string) *element {
	e.SetAttribute("media", mediaquery)
	return e
}

// Ping A space-separated list of URLs. When the area is activated, the browser will send POST requests with the body
...
// Rel The relationship between the current document and the linked document. Valid on area elements, this attribute
...
// Target Where to display the linked URL, as the name for a browsing context (a tab, window, or <iframe>). Possible
...
// Type Hints at the linked URL's format with a MIME type. No built-in functionality. Use only when href is present.
