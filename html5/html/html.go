...
//
// The <html> HTML element represents the root (top-level element) of an HTML document, so it is also referred
// to as the root element. All other elements must be descendants of this element. There can be only one <html>
...
// Text creates a complete HTML5 document with DOCTYPE declaration and text content. Uses text.Text which HTML-escapes the output.
...
// FragmentText creates an html fragment without DOCTYPE declaration with text content. Uses text.Text which HTML-escapes the output.
...
//
// The primary language of the document. Should be a valid BCP 47 language tag
func (e *element) Lang(language string) *element {
	e.SetAttribute("lang", language)
	return e
}

// Xmlns sets the xmlns attribute.
//
// Defines the XML namespace for the document. Typically "http://www.w3.org/1999/xhtml" for XHTML documents
...
func (e *element) RenderBuilder(buf *bytes.Buffer) {
	if e.doctype {
		buf.Write(html5.MarkupDoctype)
	}
	buf.Write(html5.TagHtml)
