...
//
// The <table>: The Table element represents tabular data -that is, information presented in a two-dimensional
...
// Element represents the <table> HTML element.
type Element struct {
...
// RowsOf creates a new <table> element from a slice of data and a mapper that returns one tr element per item, enforcing correct nesting at compile time.
// The mapper runs at render time inside a deferred function component - see node.Funcs for the contract.
// A nil result from the mapper is skipped, so the mapper can drop an item; use RowsFunc when the list is reordered or computed.
// Example: table.RowsOf([]int{1, 2}, func(int) *tr.Element { return tr.New() })
// Renders: <table><tr></tr><tr></tr></table>
func RowsOf[T any](items []T, fn func(T) *tr.Element) *Element {
	return &Element{
...
// RowsFunc creates a new <table> element from a function that returns tr elements, enforcing correct nesting at compile time.
...
// Example: table.RowsFunc(func() []*tr.Element { return []*tr.Element{tr.New(), tr.New()} })
// Renders: <table><tr></tr><tr></tr></table>
func RowsFunc(fn func() []*tr.Element) *Element {
	return &Element{
		nodes: []node.Node{node.Funcs(func() []node.Node {
			items := fn()
...
//
// Defines the width in pixels of the border frame surrounding the table. This is a legacy HTML attribute
// that's largely superseded by CSS border properties in modern web development. When set to 0, no border
// frame is displayed. While still supported for backwards compatibility, CSS should be preferred for styling
// table borders with more control over appearance, colors, and styles.
//
// Deprecated: Use CSS border properties instead.
func (e *Element) Border(width string) *Element {
	e.SetAttribute("border", width)
	return e
}

// CellPadding sets the cellpadding attribute.
//
// Specifies the amount of space between the cell content and the cell border in pixels or as a percentage.
// This creates internal padding within each table cell, making content more readable by preventing it from
// touching the cell borders. This is a legacy HTML attribute that's been superseded by CSS padding
// properties, which offer more precise control and better maintainability in modern web development.
//
// Deprecated: Use CSS padding properties instead.
func (e *Element) CellPadding(padding string) *Element {
	e.SetAttribute("cellpadding", padding)
	return e
}

// CellSpacing sets the cellspacing attribute.
//
// Defines the amount of space between adjacent table cells, specified in pixels or as a percentage. This
// creates gaps between cells, affecting the overall table appearance and spacing. When set to 0, cells are
// adjacent with no spacing. This is a legacy HTML attribute that's been replaced by CSS border-spacing and
// border-collapse properties in modern web development, which provide more flexible spacing control.
//
// Deprecated: Use CSS border-spacing and border-collapse properties instead.
func (e *Element) CellSpacing(spacing string) *Element {
	e.SetAttribute("cellspacing", spacing)
	return e
}

// Frame sets the frame attribute.
//
// Specifies which sides of the table frame (border) should be displayed. This legacy attribute provides basic
// control over table border appearance by selectively showing or hiding specific sides of the outer table
// border. Modern CSS border properties offer much more sophisticated control and should be used instead.
// Possible values: void (no sides), above (top side only), below (bottom side only), hsides (top and
// bottom), lhs (left side only), rhs (right side only), vsides (left and right), box (all sides), border
// (all sides)
//
// Deprecated: Use CSS border properties instead.
func (e *Element) Frame(sides string) *Element {
	e.SetAttribute("frame", sides)
	return e
}

// Rules sets the rules attribute.
//
// Specifies where dividing lines (rules) should appear between table cells. This legacy attribute controls
// the internal grid lines within the table structure, helping to visually separate different sections of
// tabular data. Modern CSS border properties provide much more flexible and maintainable ways to achieve
// similar effects.
// Possible values: none (no rules), groups (rules between row groups and column groups), rows (rules between
// rows), cols (rules between columns), all (rules between all rows and columns)
//
// Deprecated: Use CSS border properties instead.
func (e *Element) Rules(rules string) *Element {
	e.SetAttribute("rules", rules)
	return e
}

// Summary sets the summary attribute.
//
// Provides a textual summary of the table's content and structure for accessibility purposes. This attribute
// was intended to help screen readers and other assistive technologies understand complex tables before
// processing the detailed cell content. However, this attribute has been deprecated in HTML5 in favor of
// more semantic approaches like caption elements, aria-label, or aria-describedby attributes for better
// accessibility.
//
// Deprecated: Use caption, aria-label, or aria-describedby instead.
func (e *Element) Summary(text string) *Element {
	e.SetAttribute("summary", text)
	return e
}

// Width sets the width attribute.
//
// Specifies the width of the table, either as a fixed pixel value (e.g., '500') or as a percentage of the
// containing element (e.g., '100%'). This legacy attribute has been largely superseded by CSS width
// properties, which provide more sophisticated layout control, responsive design capabilities, and better
// integration with modern layout systems like flexbox and grid.
//
// Deprecated: Use CSS width properties instead.
