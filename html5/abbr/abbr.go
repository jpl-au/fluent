...
//
// The <abbr> element marks an abbreviation or acronym. Its title attribute can
// supply the full expansion, but a visible expansion on first use makes the
// meaning available without relying on a tooltip.
package abbr
...
// Titled creates an abbreviation with its expansion in the title attribute.
// Browsers commonly show the expansion on hover. Include a visible expansion
// when readers need it, since a tooltip is not available in every interaction mode.
...
//
// The full expansion of the abbreviation, which may be presented as a tooltip.
// Each abbr element has its own title. Include a visible expansion when readers
// need it, since a tooltip is not available through every input method.
func (e *Element) Title(expansion string) *Element {
	e.title = node.EscapeAttribute(expansion)
	return e
}
