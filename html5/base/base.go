...
//
// The <base> element sets the base URL for resolving relative URLs and the
// default target for links and forms. Place it in head before elements whose
// URLs depend on it. Only one base element is permitted per document.
package base
...
//
// The base URL used to resolve relative URLs in the document. A relative base
// URL is itself resolved against the document's fallback base URL. Place the
// base element before resources and links whose URLs depend on it.
//
// Fluent filters this URL at render time: http, https, mailto, tel, sms and relative URLs
// render as given; any other scheme (javascript:, data:, file:, a custom protocol) renders as the
// inert node.UnsafeURL sentinel instead. For a trusted off-allowlist value use SetAttribute
...
//
// Specifies the default browsing context for all hyperlinks and forms in the document. This sets where links
// open by default unless overridden by individual elements. Common values: '_self' (same frame), '_blank'
// (new tab/window), '_parent' (parent frame), '_top' (full window), or a named frame/window.
func (e *Element) Target(context target.Target) *Element {
	e.target = context
	return e
}
