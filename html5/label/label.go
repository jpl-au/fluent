...
//
// The <label> HTML element provides a caption for a form control, improving usability and accessibility.
// Associated with a control via the for attribute or by wrapping the control. Clicking the label focuses or
// activates the associated control. Essential for accessibility as screen readers announce the label text,
// and improves user experience by expanding clickable areas for checkboxes and radio buttons.
package label
...
//
// The id of the form control that this label is associated with. When set, clicking the label will focus
// or activate the associated control. This association is crucial for accessibility, allowing screen readers
// to properly connect labels with their controls and improving usability by making labels interactive.
func (e *Element) For(forID string) *Element {
	e.forID = node.EscapeAttribute(forID)
	return e
}
...
func (e *Element) AttributeBuilder(buf *bytes.Buffer) {
	if e.forID != "" {
		buf.Write(html5.AttrFor)
		buf.WriteString(e.forID)
