...
//
// The <colgroup> element groups table columns for shared presentation. Use its
// span attribute for a group without col children, or supply col children to
// describe individual columns. The two forms must not be combined.
package colgroup
...
// Element represents the <colgroup> HTML element.
type Element struct {
...
//
// Specifies the number of consecutive columns the <colgroup> element spans. The value must be a positive
// integer greater than zero. If not present, its default value is 1. This attribute provides a shorthand
// way to apply styling to multiple columns without requiring individual <col> elements. Important: The span
...
func (e *Element) RenderBuilder(buf *bytes.Buffer) {
	...
	buf.Write(html5.TagColgroupClose)
}
