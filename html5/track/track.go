...
//
// Specifies the URL of the track file, typically a WebVTT (.vtt) file containing time-synchronized text data.
// The URL must be valid and, due to security restrictions, must have the same origin as the document unless CORS
// is properly configured. This attribute is required for the track element to function. The track file contains
// timed cues that are displayed or processed in sync with the media playback.
...
//
// Specifies how the text track should be used by the media player, determining its purpose and how it's
// presented to users. This affects both the user interface (what controls are shown) and the default behavior
// of the track. If omitted, the default kind is 'subtitles'.
// Possible values: subtitles (translations for foreign language content), captions (transcription including
// sound effects for deaf/hard-of-hearing users), descriptions (audio descriptions for visually impaired users),
// chapters (navigation cues), metadata (not displayed to users, used by scripts)
func (e *Element) Kind(kind string) *Element {
	e.kind = node.EscapeAttribute(kind)
	return e
}

// Label sets the label attribute.
//
// Provides a human-readable title for the text track that appears in the browser's track selection menu. This
// label helps users identify and choose between multiple available tracks, such as different subtitle languages
// or caption options. Should be descriptive and clear, like 'English Subtitles' or 'French Captions'.
// Essential for user experience when multiple tracks are available.
func (e *Element) Label(text string) *Element {
	e.label = node.EscapeAttribute(text)
	return e
}
...
//
// Enables this track by default when the media loads. Only one track of each kind should have this
// attribute. Provides a fallback selection when multiple tracks are available.
//
// Calling without an argument sets the attribute. Pass a bool to control presence conditionally.
func (e *Element) Default(conds ...bool) *Element {
	if len(conds) > 0 {
		e.isDefault = conds[0]
	} else {
		e.isDefault = true
...
func (e *Element) AttributeBuilder(buf *bytes.Buffer) {
	...
	if e.kind != "" {
		buf.Write(html5.AttrKind)
		buf.WriteString(e.kind)
		buf.Write(html5.MarkupQuote)
	}
	if e.label != "" {
		buf.Write(html5.AttrLabel)
