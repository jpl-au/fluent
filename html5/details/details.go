...
//
// The <details> HTML element creates a disclosure widget in which information is visible only when the widget
// is toggled into an open state. A summary or label must be provided using the <summary> element.
package details
...
// Element represents the <details> HTML element.
type Element struct {
...
// Renders: <details name="faq"><summary>Question 1</summary><p>Answer 1</p></details>
...
//
// Marks the details widget as expanded, making the content visible. When absent, the content is hidden.
// Users can toggle this state by clicking on the summary element.
//
// Calling without an argument sets the attribute. Pass a bool to control presence conditionally.
func (e *Element) Open(conds ...bool) *Element {
	if len(conds) > 0 {
		e.open = conds[0]
	} else {
		e.open = true
...
//
// This attribute creates a group name for multiple <details> elements, ensuring only one element in the
// group can be open at a time. When a user opens one details element in the group, all others with the
// same name automatically close. This enables accordion-style interfaces without requiring JavaScript,
// providing native browser support for mutually exclusive disclosure widgets.
...
func (e *Element) RenderBuilder(buf *bytes.Buffer) {
	...
	buf.Write(html5.TagDetailsClose)
}
