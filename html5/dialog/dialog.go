...
//
// The HTML <dialog> element is used to create both modal and non-modal dialog boxes. Modal dialog boxes
// interrupt interaction with the rest of the page being inert, while non-modal dialog boxes allow interaction
...
// Element represents the <dialog> HTML element.
type Element struct {
...
func Open(nodes ...node.Node) *Element {
	return &Element{
		nodes: nodes,
		open:  true,
	}
...
//
// Marks the dialog as active and visible for interaction. When absent, the dialog is hidden. Use
// JavaScript showModal() for modal dialogs; this attribute is for non-modal dialogs.
//
...
// "any", "closerequest", and "none".
func (e *Element) ClosedBy(closedby string) *Element {
	e.closedBy = node.EscapeAttribute(closedby)
	return e
}
...
func (e *Element) AttributeBuilder(buf *bytes.Buffer) {
	...
	if e.closedBy != "" {
		buf.Write(html5.AttrClosedBy)
		buf.WriteString(e.closedBy)
...
func (e *Element) RenderBuilder(buf *bytes.Buffer) {
	...
	buf.Write(html5.TagDialogClose)
}
