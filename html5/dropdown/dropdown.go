...
//
// The <select> element offers a choice from option children, optionally grouped
// with optgroup. Multiple permits several selections. Name identifies the
// selected values in form submission.
package dropdown
...
//
// The name used for submitted selections. With multiple, each selected option
// contributes a separate form entry with this name.
...
// submissions. Useful for conditional form logic where certain selections are temporarily unavailable
...
//
// The id of the form that owns the select. It must identify a form in the same
// document and can associate a control placed outside the form with it.
...
// box showing multiple options at once rather than a dropdown. Changes the form submission behaviour to send
...
//
// Requires an option to be selected during constraint validation. For a single
// select displayed as one row, a first direct-child option with an empty value
// acts as a placeholder and does not satisfy the requirement.
//
// Calling without an argument sets the attribute. Pass a bool to control presence conditionally.
...
//
// The requested number of visible options. A value greater than one normally
// produces a scrolling list rather than a collapsed menu. Browser presentation
// also depends on multiple and platform conventions.
