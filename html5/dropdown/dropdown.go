...
// OptionsOf creates a new <select> element from a slice of data and a mapper that returns one option element per item, enforcing correct nesting at compile time.
...
// Example: dropdown.OptionsOf([]int{1, 2}, func(int) *option.Element { return option.New() })
// Renders: <select><option></option><option></option></select>
...
// OptionsFunc creates a new <select> element from a function that returns option elements, enforcing correct nesting at compile time.
...
// Example: dropdown.OptionsFunc(func() []*option.Element { return []*option.Element{option.New(), option.New()} })
// Renders: <select><option></option><option></option></select>
...
//
// Specifies the name of the select element for form submission and programmatic access. This name is used
// to identify the selected value(s) in form data when submitted to the server. The name appears as the key
// in form submissions (e.g., 'country=us'). Essential for server-side form processing and JavaScript form
// manipulation via form.elements API.
...
// this select element. This enhances user experience by suggesting relevant options based on browsing history
// and form patterns. Common values include 'on', 'off', or specific autocomplete tokens like 'country',
// 'language', or 'sex' for semantic autocompletion.
...
//
// A boolean attribute that disables the select element, preventing user interaction and form submission. When
// disabled, the element appears grayed out, cannot receive focus, and its value is not included in form
// submissions. Useful for conditional form logic where certain selections should be temporarily unavailable
// based on other form values or application state.
//
...
//
// Associates the select element with a form element in the document by referencing the form's ID attribute.
// This allows the select to participate in form submission and validation even when not physically nested
...
func (e *Element) Form(form_id string) *Element {
	e.form = node.EscapeAttribute(form_id)
	return e
}

// Multiple sets the multiple attribute.
//
// A boolean attribute that allows users to select multiple options simultaneously using Ctrl+click (or
// Cmd+click on Mac) and keyboard navigation. When present, the select element typically displays as a list
// box showing multiple options at once rather than a dropdown. Changes the form submission behavior to send
// multiple values for the same name, and affects validation and accessibility features.
//
// Calling without an argument sets the attribute. Pass a bool to control presence conditionally.
...
//
// A boolean attribute that makes this select element mandatory for form submission. Browsers will prevent form
// submission and display validation messages if no option is selected (or if the first option has an empty
// value and is selected). Works with browser built-in form validation and provides automatic validation
// feedback to users. Essential for ensuring required data is collected in forms.
//
// Calling without an argument sets the attribute. Pass a bool to control presence conditionally.
...
//
// Specifies the number of options that should be visible in the select element without scrolling. When size
// is greater than 1, the select appears as a list box rather than a dropdown menu. This affects the visual
...
func (e *Element) RenderBuilder(buf *bytes.Buffer) {
	...
	buf.Write(html5.TagSelectClose)
}
