...
// Element represents the <math> HTML element.
type Element struct {
...
//
// Specifies how the enclosed MathML markup should be rendered. Can be 'block' for displayed equations or
// 'inline' for inline formulas.
func (e *Element) Display(value string) *Element {
	e.display = node.EscapeAttribute(value)
	return e
}
...
//
// Specifies the XML namespace for MathML. Typically set to "http://www.w3.org/1998/Math/MathML".
func (e *Element) Xmlns(namespace string) *Element {
	e.xmlns = node.EscapeAttribute(namespace)
	return e
}
...
func (e *Element) AttributeBuilder(buf *bytes.Buffer) {
	if e.display != "" {
		buf.Write(html5.AttrDisplay)
		buf.WriteString(e.display)
		buf.Write(html5.MarkupQuote)
	}
	if e.mathVariant != "" {
		buf.Write(html5.AttrMathVariant)
		buf.WriteString(e.mathVariant)
		buf.Write(html5.MarkupQuote)
	}
	if e.xmlns != "" {
		buf.Write(html5.AttrXmlns)
